...
class IndicTranslation(DistributedDataClassifier):
    ...
    def process_input_text(self, df: cudf.DataFrame) -> cudf.DataFrame:
        ...
        print(f"df[doc_id] : {df['doc_id'].unique()}, df[adl_id] : {df['adlr_id'].unique()}")
    ...
    def grouping(self, df: cudf.DataFrame) -> cudf.DataFrame:
        ...
        other_columns = {
            ...
            if col not in agg_funcs and col != "adlr_id"
        }

        agg_funcs.update(other_columns)
        df = df.groupby("adlr_id").agg(agg_funcs).reset_index()
        df = cudf.DataFrame.from_pandas(df)
        return df

    def partition_memory_size(self, df):
        return df.memory_usage(deep=True).sum()

    def _run_classifier(self, dataset: DocumentDataset) -> DocumentDataset:
        ddf = dataset.df
        ddf_all = ddf[['text','filename','adlr_id','has_letters']]

        ddf = ddf_all[ddf_all["has_letters"]]

        # do start mid end split here via cudf.
        # ^([^a-zA-Z0-9]*)([a-zA-Z0-9]{,2}|[a-zA-Z0-9].*[a-zA-Z0-9])([^a-zA-Z0-9]*$)
        # tcdf = ddf["text"].str.extract(r'^([^a-zA-Z0-9]*)(.*)([^a-zA-Z0-9]*$)')
        tcdf = ddf["text"].str.extract(r'^([^a-zA-Z0-9]*)([a-zA-Z0-9]{,2}|[a-zA-Z0-9].*[a-zA-Z0-9])([^a-zA-Z0-9]*$)')
        tcdf_s = tcdf[0].str.extract(r'(.*)([({\[\"\'\u2018\u201c]*)$')
        tcdf_e = tcdf[2].str.extract(r'^([.!?:,;)}\]\'\"”’]*)(.*)$')

        ddf["text"] = tcdf_s[1].fillna('')+tcdf[1].fillna('')+tcdf_e[0].fillna('')
        ddf["start_sym"] = tcdf_s[0].fillna('')
        ddf["end_sym"] = tcdf_e[1].fillna('')
        # del tcdf
        # del tcdf_s
        # del tcdf_e
        ...
        preprocessed_meta = ddf._meta.copy()
        preprocessed_meta['indic_proc_text'] = ''

        ddf = ddf.map_partitions(self.preprocess_df,meta=preprocessed_meta)
        ...
        ddf_all = ddf_all.drop(columns=["false_translation", "has_letters"])
        ddf_all = ddf_all.map_partitions(self.grouping)
...
def main(args):
    ...
    print(f"len(input files) : {len(input_files)}")
    
    from dask.distributed import performance_report

    with performance_report(filename="/lustre/fsw/portfolios/llmservice/users/uahmed/mfsp_dask-report.html"):
        st = time.time()
